...
class schedule
{
   ...
   public:
    ...
    schedule& append(const schedule<N, E>& S)
    {
 ...
 */
template <typename N, typename E>
inline std::ostream& operator<<(std::ostream& file, const schedule<N, E>& S)
{
    std::string sep = "";
 ...
 * @return schedule<N, E> the deep first schedule of G
 */
template <typename N, typename E>
inline schedule<N, E> dfschedule(const digraph<N, E>& G)
{
    schedule<N, E> S;
 ...
 * @param G the graph we want to schedule
 * @return schedule<N, E> the breadth first schedule of G
 */

template <typename N, typename E>
inline schedule<N, E> bfschedule(const digraph<N, E>& G)
{
    std::vector<std::vector<N>> P = rparallelize(G);
    schedule<N, E>                 S;
 ...
 */
template <typename N, typename E>
inline int schedulingcost(const digraph<N, E>& G, const schedule<N, E>& S)
{
    int cost = 0;
 ...
 */
template <typename N, typename E>
inline schedule<N, E> dfcyclesschedule(const digraph<N, E>& G)
{
    digraph<digraph<N, E>, E>  H  = graph2dag(G);
    schedule<digraph<N, E>, E> SH = dfschedule(H);
    schedule<N, E>          S;
    for (const digraph<N, E>& n : SH.elements()) { S.append(dfschedule(cut(n, 1))); }
    return S;
}
 ...
 */
template <typename N, typename E>
inline schedule<N, E> bfcyclesschedule(const digraph<N, E>& G)
{
    digraph<digraph<N, E>, E>  H  = graph2dag(G);
    schedule<digraph<N, E>, E> SH = bfschedule(H);
    schedule<N, E>          S;
    for (const digraph<N, E>& n : SH.elements()) { S.append(dfschedule(cut(n, 1))); }
    return S;
}
