...
// USAGE : Tarjan<N, E> t(mygraph); ...t.partition()...
//===========================================================
//===========================================================
...
class Tarjan
{
   ...
   public:
    explicit Tarjan(const digraph<N, E>& g) : fGraph(g), fGroup(0), fCycleCount(0)
    {
...
template <typename N, typename E>
inline int cycles(const digraph<N, E>& g)
{
    Tarjan<N, E> T(g);
    return T.cycles();
}
...
template <typename N, typename E>
inline digraph<digraph<N, E>, E> graph2dag(const digraph<N, E>& g)
{
    Tarjan<N, E>               T(g);  // the partition of g
    std::map<N, digraph<N, E>> M;     // std::mapping between nodes and supernodes
    digraph<digraph<N, E>, E>     sg;    // the resulting supergraph
    ...
    for (const auto& s : T.partition()) {
        digraph<N, E> sn;                        // the supernode graph
    ...
    for (const auto& n1 : g.nodes()) {
        digraph<N, E> sn1(M[n1]);
        for (const auto& c : g.connections(n1)) {
            digraph<N, E> sn2(M[c.first]);
            if (sn1 == sn2) {
...
template <typename N, typename E>
inline digraph<digraph<N, E>, E> graph2dag2(const digraph<N, E>& g)
{
    Tarjan<N, E>               T(g);  // the partition of g
    std::map<N, digraph<N, E>> M;     // std::mapping between nodes and supernodes
    digraph<digraph<N, E>, E>     sg;    // the resulting supergraph
    std::map<std::pair<digraph<N, E>, digraph<N, E>>, int> CC;  // count of connections between supernodes
    ...
    for (const auto& n1 : g.nodes()) {             // for each node n1
        digraph<N, E> sn1(M[n1]);                     // retrieve the supernode
...
template <typename N, typename E>
inline std::vector<std::vector<N>> parallelize(const digraph<N, E>& g)
{
    //-----------------------------------------------------------
...
template <typename N, typename E>
inline std::vector<std::vector<N>> rparallelize(const digraph<N, E>& G)
{
...
template <typename N, typename E>
inline std::vector<N> serialize(const digraph<N, E>& G)
{
    //------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------
    using Visitfun =
        std::function<void(const digraph<N, E>&, const N&, std::set<N>&, std::vector<N>&)>;
    Visitfun visit = [&visit](const digraph<N, E>& g, const N& n, std::set<N>& V, std::vector<N>& S) {
        if (V.find(n) == V.end()) {
            V.insert(n);
...
template <typename N, typename M, typename E>
inline digraph<M, E> mapnodes(const digraph<N, E>& g, std::function<M(const N&)> foo)
{
    digraph<M, E>     r;
    std::map<N, M> cache;
...
template <typename N, typename E>
inline digraph<N, E> reverse(const digraph<N, E>& g)
{
    digraph<N, E> r;
    // copy the connections
    for (const auto& n : g.nodes()) {
...
template <typename N, typename E>
inline digraph<N, E> mapconnections(const digraph<N, E>&                            G,
 ...
 */
template <typename N, typename E>
void splitgraph(const digraph<N, E>& G, std::function<bool(const N&)> left, digraph<N, E>& L,
                digraph<N, E>& R)
{
    for (auto n : G.nodes()) {
        if (left(n)) {
 ...
 */
template <typename N, typename E>
digraph<N, E> subgraph(const digraph<N, E>& G, const std::set<N>& S)
{
    digraph<N, E>  R;     // the (R)esulting graph
...
template <typename N, typename E>
inline digraph<N, E> cut(const digraph<N, E>& G, int dm)
{
...
template <typename N, typename E>
inline digraph<N, E> chain(const digraph<N, E>& g, bool strict)
{
    const digraph<N, E> h = reverse(g);
    digraph<N, E>       r;
    for (const auto& n : g.nodes()) {
        if (!strict) r.add(n);
...
template <typename N, typename E>
inline std::vector<N> roots(const digraph<N, E>& G)
{
    std::map<N, int> R;
    for (const N& n : G.nodes()) {
...
template <typename N, typename E>
inline std::vector<N> leaves(const digraph<N, E>& G)
{
    std::vector<N> L;
    for (const N& n : G.nodes()) {
...
template <typename N, typename E>
inline std::ostream& operator<<(std::ostream& file, const digraph<N, E>& g)
{
    std::string sep = "";
...
template <typename N, typename E>
inline std::ostream& dotfile(std::ostream& file, const digraph<N, E>& g, bool clusters = false)
{
